...
import re
try:
    from intent import Loki_drink
    from intent import Loki_idiom
    from intent import Loki_n_n
    from intent import Loki_adj_adj
    from intent import Loki_action
except:
    from .intent import Loki_drink
    from .intent import Loki_idiom
    from .intent import Loki_n_n
    ...
    if lokiRst.getStatus():
        for index, key in enumerate(inputLIST):
            ...
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                # drink
                if lokiRst.getIntent(index, resultIndex) == "drink":
                    lokiResultDICT = Loki_drink.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)

                # idiom
                if lokiRst.getIntent(index, resultIndex) == "idiom":
                    lokiResultDICT = Loki_idiom.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)

                # n_n
                if lokiRst.getIntent(index, resultIndex) == "n_n":
                    lokiResultDICT = Loki_n_n.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)

                # adj_adj
                if lokiRst.getIntent(index, resultIndex) == "adj_adj":
                    lokiResultDICT = Loki_adj_adj.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)

                # action
                if lokiRst.getIntent(index, resultIndex) == "action":
                    lokiResultDICT = Loki_action.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)
...
def testIntent():
    # drinks
    print("[TEST] drinks")
    inputLIST = ['微糖微冰','中杯冰拿鐵','大杯冰奶茶','大杯冰紅茶','無糖熱可可']
    testLoki(inputLIST, ['drinks'])
    print("")

    # idiom
    print("[TEST] idiom")
    inputLIST = ['精神抖擻']
    testLoki(inputLIST, ['idiom'])
    print("")

    # n_n
    print("[TEST] n_n")
    inputLIST = ['玉米蛋餅']
    testLoki(inputLIST, ['n_n'])
    print("")

    # adj_adj
    print("[TEST] adj_adj")
    inputLIST = ['可愛可惡']
    testLoki(inputLIST, ['adj_adj'])
    print("")

    # action
    print("[TEST] action")
    inputLIST = ['發生關係']
    testLoki(inputLIST, ['action'])
    print("")
